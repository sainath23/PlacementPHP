...
<form action="act_admin_add_state.php" method="post" name="AddState">
<table border="0" align="center">
  <tbody>
  <tr>
  <td><label for="selectCountryName"><strong>Country Name:</strong></label></td>
  <td><select name="selectCountryName" id="selectCountryName">
  <option id="0">Select</option>
  <?php
              $con = mysqli_connect("localhost", "root", "", "placement") 
              or die("Failed to connect MySQL: " . mysqli_error());

              $query = "SELECT * FROM tbl_Country_Master";
              $result = mysqli_query($con, $query) or die(mysqli_error());
              
              while ($row=mysqli_fetch_array($result)) 
              {
				  ?>
                  <option id="<?php echo $row['CountryId']; ?>"><?php echo $row['CountryName']; ?></option>
                  <?php }
				  mysqli_close($con);
                  ?>
    </select></td>
    ...
    <tr>
      <td><label for="txtStateName"><strong>State Name:</strong></label></td>
      <td>
        <input type="text" name="txtStateName" id="txtStateName"></td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="2"><input type="checkbox" name="checkbox" id="checkbox">
        <label for="checkbox">State Info (Check the check option to view existed data)</label></td>
    </tr>
    <tr>
